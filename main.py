"""
============================================================
🎬 FULLY AUTOMATED INSTAGRAM REELS UPLOADER (HINDI)
============================================================
"""
...
class C:
    ...
    LANGUAGE      = os.environ.get("CONTENT_LANGUAGE", "hindi").lower()
...
def _sanitize(msg):
    for secret in [C.IG_PASS, C.DRIVE_KEY, C.GEMINI_KEY, C.IG_SESSION[:20] if C.IG_SESSION else ""]:
        if secret and len(secret) > 5 and secret in str(msg):
            msg = str(msg).replace(secret, "***")
    return msg

def log(msg, prefix="✅"):
    print(f"[{datetime.now().strftime('%H:%M:%S')}] {prefix} {_sanitize(msg)}", flush=True)
...
def git_push():
    ...
    ignore_entries = [
        ...
        "detailed_log.txt", "current_movie.mp4"
    ]
    existing = ""
    ...
    while True:
        ...
        try:
            ...
            if r.status_code != 200:
                log_err(f"Drive API {r.status_code}")
                return []
            data = r.json()
        ...
        try:
            ...
            if os.path.exists(out_path) and os.path.getsize(out_path) > 10000:
                ...
                if b"<!doctype" in head or b"<html" in head:
                    log_err("Got HTML instead of video — file too large for API key")
                    os.remove(out_path)
                    return False
...
# ── CAPTIONS (Gemini first, Hindi fallback) ───────────────────
    ...
    if GEMINI and C.GEMINI_KEY:
        try:
            ...
            prompt = (
                ...
                f"Style: {style}\n\n"
                f"STRICT RULES:\n"
                f"1. Write the main text in {lang} script (Hindi/Telugu/Tamil native script, NOT English transliteration)\n"
                f"2. Start with 1-2 emojis + a {style}\n"
                f"3. Mention Part {p}/{t} naturally in the text\n"
            ...
            for model in C.GEMINI_MODELS:
                try:
                    ...
                    log_warn(f"Gemini caption {model}: {str(e)[:80]}")
                    continue
        except Exception as e:
            log_warn(f"Gemini caption init: {str(e)[:80]}")

    log_warn("Gemini unavailable — using fallback caption")

    fallback = {
        "hindi": [
            f"😱 {name} देखो!\nPart {p}/{t}\n\nNext part के लिए Follow करो! 🔔\n.\n.\n.\n"
            f"#doraemon #hindi #hindicartoon #hindireels #viral #trending #fyp #reels "
            f"#anime #cartoon #doraemonhindi #cartoonhindi #hindianimation "
            f"#doraemonhindinew #cartooninhindi",

            f"🔥 {name} — Part {p}/{t}\n\nLike ❤️ और Follow करो!\n.\n.\n.\n"
            f"#doraemon #hindi #trending #viral #reels #fyp #cartoonhindi "
            f"#hindianimation #hindicartoon #anime #doraemonhindi #hindireels "
            f"#doraemonnewepisode #cartoonsinhindi",

            f"🎬 {name} [{p}/{t}]\n\nपसंद आया तो Like करो! 👇\nFollow for more! 🔔\n.\n.\n.\n"
            f"#hindi #doraemon #cartoon #viral #trending #reels #fyp "
            f"#hindicartoon #anime #doraemonhindi #cartoonhindi #hindireels "
            f"#doraemoninhindi #animehindi",

            f"🍿 {name}\nPart {p} of {t}\n\nComment में बताओ कैसा लगा! 💬\nNext part कल! ⏭️\n.\n.\n.\n"
            f"#doraemon #hindi #viral #reels #trending #fyp #hindicartoon "
            f"#anime #cartoon #doraemonhindi #hindireels #hindianimation "
            f"#doraemonnewepisode #cartoonsinhindi",

            f"😍 {name} | Part {p}/{t}\n\nDosto ko Share करो! 🫂\nFollow करो next part के लिए! 🔔\n.\n.\n.\n"
            f"#doraemon #hindi #cartoon #viral #trending #reels #fyp "
            f"#hindianimation #anime #cartoonhindi #doraemonhindi #hindireels "
            f"#doraemoninhindi #animehindi",

            f"🤩 {name} — Part {p}/{t}\n\nये episode देखा क्या? 😱\nLike और Follow ज़रूर करो!\n.\n.\n.\n"
            f"#doraemon #hindi #hindicartoon #viral #trending #reels #fyp "
            f"#anime #cartoon #doraemonhindi #cartoonhindi #hindireels "
            f"#doraemonnewepisode #hindianimation",

            f"🎭 {name} Part {p}/{t}\n\nDoraemon का ये gadget कमाल है! 🚀\nFollow for daily episodes! 🔔\n.\n.\n.\n"
            f"#doraemon #hindi #cartoon #viral #trending #reels #fyp "
            f"#hindicartoon #anime #doraemonhindi #cartoonhindi #hindireels "
            f"#cartoonsinhindi #animehindi",
        ],
        "telugu": [
            f"😱 {name} చూడండి!\nPart {p}/{t}\n\nFollow చేయండి! 🔔\n.\n.\n.\n"
...
def setup():
    ...
    missing = []
    for val, name in [
        (C.IG_USER, "IG_USERNAME"), (C.IG_PASS, "IG_PASSWORD"),
    ...
    log(f"  {'✓' if C.WATERMARK else '~'} WATERMARK")
...
def main():
    ...
    if progress.get("drive_id") != drive_id:
        ...
        progress = {"drive_id": drive_id, "part": 0, "total": total, "thumb_time": -1, "cooldown_until": ""}
    last = progress["part"]
    log(f"Progress: {last}/{total} done — next is Part {last + 1}")
    ...
    if result is True:
        ...
        if part >= total:
            ...
            ci = order.index(drive_id) if drive_id in order else -1
            if ci >= 0 and ci + 1 < len(order):
                nm = id_map.get(order[ci + 1])
                if nm:
                    log(f"⏭️ Next: {nm['display']}")
            else:
                log("🏆 LAST video done!")
        else:
            log(f"{total - part} parts left (~{(total - part) * 2}h at 12/day)")
    else:
        video_info["errors"] = video_info.get("errors", 0) + 1
    ...
    print("\n" + "=" * 50, flush=True)
    print(f"📊 Episodes: {log_data.get('completed', 0)}/{len(log_data['videos'])} | "
          f"Reels: {log_data.get('uploaded', 0)}", flush=True)
    print("=" * 50, flush=True)
